...
A requested payment shoudl really almost always result in exactly one payment. Not zero payments and also not two or more payments.

Similar the information we send should reach the accounting system once. I would be acceptable to send it twice, but the information shoudl almost never get lost.
